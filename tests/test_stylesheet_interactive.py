"""Exercises every QSS builder in ``stylesheet_interactive`` against the real palette.
...
BUILDERS = [
    (_qss_buttons, "QPushButton#primary"),
    (_qss_sidebar, "QLabel#sidebarBrand"),
    (_qss_log_toolbar, "QFrame#logToolbar"),
    (_qss_status_bar, "QWidget#statusBarWidget"),
]
...
def test_primary_button_uses_accent():
    # The primary button must paint with the accent token (DESIGN.md contract).
    qss = _qss_buttons(COLORS)
    assert COLORS["accent"] in qss.split("QPushButton#primary")[1]
