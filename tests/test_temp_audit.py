...
from src.agent_tools.temp_audit import _format_size, scan_temp_folders, analyze_temp_folders, clean_temp_folders
...
@patch('src.agent_tools.temp_audit.scan_dir_size')
def test_scan_temp_folders_empty_dirs(mock_scan_dir):
    mock_scan_dir.return_value = (0, 0)
    result = scan_temp_folders()
    assert result["total_bytes"] == 0
    assert all(d["file_count"] == 0 for d in result["details"].values())
